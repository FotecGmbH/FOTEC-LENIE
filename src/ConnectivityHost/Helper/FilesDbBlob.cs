...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
        ...
        public static async Task DeleteFile(Db db, long fileId)
        {
            if (db == null!)
            {
                throw new NullReferenceException("[DeleteFile] Database context is NULL!");
            }

            var tblFile = db.TblFiles.FirstOrDefault(x => x.Id == fileId);
            Logging.Log.LogTrace($"[DeleteFile] Id {fileId}");
            if (tblFile == null)
            {
                throw new FileNotFoundException($"[DeleteFile] File with Id {fileId} does not exist in database!");
            }
            ...
            if (await fileHelper.ExistsFile(tblFile.BlobName).ConfigureAwait(true))
            {
                ...
                if (!res)
                {
                    throw new ApplicationException($"[DeleteFile] Could not Delete File whit the name {tblFile.BlobName} in Blob");
                }
            }
