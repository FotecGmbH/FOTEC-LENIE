...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
            ...
            // Userdaten an Client schicken - sofern DeviceId gesetzt und Client verbunden
            try
            {
                if (deviceId > 0 && _hub.ClientConnection.GetClients().Any(x => x.DeviceId == deviceId))
                {
            ...
            {
                Logging.Log.LogError($"[{GetType().Name}]({nameof(UserValidateEMail)}): {e}");
            }
