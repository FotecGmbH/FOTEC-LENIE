...
namespace ConnectivityHost.BaseApp.ViewModel
{
    ...
    public class VmMessage : VmProjectBase
    {
        ...
        ///     Nachrichten Text
        /// </summary>
        public VmEntry TitleEntry { get; set; } = new(title: "Überschrift");

        /// <summary>
        ///     Nachrichten Text
        /// </summary>
        public VmEntry MessageEntry { get; set; } = new(title: "Nachricht");
...
#pragma warning restore CS0618 // Type or member is obsolete
                    ...
                    case SendViaEnum.Email:
                        ...
                        foreach (var user in Data.Users)
                        {
                            ...
                            // Email wird gesendet
                            var sendResult = await bem.SendHtmlEMail(WebSettings.Current().SendEMailAs, new List<string> {user.LoginName}, subject, htmlRendered + receiverBetaInfo, WebSettings.Current().SendEMailAsDisplayName, ccReceiver).ConfigureAwait(true);
                            if (sendResult)
                            {
                                sentMails++;
                            }
