...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        ///     Filter Entry
        /// </summary>
        public VmEntry FilterEntry { get; set; }
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
                        ...
                        {
                            var dbUser = await db.TblUsers.Where(i => i.Id == tmpUserId).Include(x => x.TblUserImage).Include(y => y.TblDevices).FirstOrDefaultAsync().ConfigureAwait(false);
            ...
            CmdDcNotifyAll = new VmCommand("Alle Benachrichtigen (via Dc)", async () =>
            {
                ...
                _ = await Nav.ToViewWithResult(typeof(VmMessage), new SendMessageData {Devices = devices.ToList(), SendVia = SendViaEnum.Dc}).ConfigureAwait(true);
            ...
            CmdDcNotify = new VmCommand("", async obj =>
            {
                if (obj is TableDevice device)
                {
                    ...
                    _ = await Nav.ToViewWithResult(typeof(VmMessage), new SendMessageData {Devices = new List<TableDevice> {device}, SendVia = SendViaEnum.Dc}).ConfigureAwait(true);
                }
...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
                ...
                if (devices.Any())
                {
                    _ = await Nav.ToViewWithResult(typeof(VmMessage), new SendMessageData {Devices = devices.ToList(), SendVia = SendViaEnum.Push}).ConfigureAwait(true);
                }
            ...
            CmdPushNotify = new VmCommand("", async selectedDevice =>
            {
                ...
                {
                    _ = await Nav.ToViewWithResult(typeof(VmMessage), new SendMessageData {Devices = new List<TableDevice> {device}, SendVia = SendViaEnum.Push}).ConfigureAwait(true);
                }
            }, "Benachrichtigen (via Push)", Glyphs.Alarm_bell);
            FilterEntry = new VmEntry(EnumVmEntryBehavior.Instantly, "Filter", "Filter", validateFunc: FilterValidation);
        ...
        /// <returns></returns>
        private (string, bool) FilterValidation(string input)
        {
            RefreshData().ConfigureAwait(true);
            return ("", true);
        }
